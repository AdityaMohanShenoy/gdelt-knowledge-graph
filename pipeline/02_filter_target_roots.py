...
DEFAULT_OUTPUT_PATH = ROOT / "data" / "working" / "india-2024" / "target_events.parquet"
DEFAULT_MANIFEST_PATH = ROOT / "data" / "working" / "india-2024" / "target_manifest.json"
...
def summarize_relation(connection: duckdb.DuckDBPyConnection, relation: str) -> dict:
    ...
    root_counts = dict(
        connection.execute(
            f"SELECT EventRootCode, COUNT(*)::BIGINT FROM {relation} GROUP BY EventRootCode"
        ).fetchall()
    )
    return {
        "rows": summary[0],
...
def build_dataset(
    ...
    try:
        ...
        output_query = f"SELECT * FROM {input_relation} WHERE EventRootCode IN ({target_codes})"
    ...
    manifest = {
        "manifest_version": "india-target-events.v1",
        "source_manifest": "india-event-universe.v1",
        "input_path": display_path(Path(input_path)),
        "output_path": display_path(output_path),
        "filter": {
            "field": "EventRootCode",
            "operator": "IN",
            "values": list(TARGET_ROOT_CODES),
        },
        ...
        "input_root_counts": input_summary["root_counts"],
        "output_root_counts": output_summary["root_counts"],
...
def main() -> None:
    args = parse_args()
    manifest = build_dataset(args.input, Path(args.output), Path(args.manifest))
